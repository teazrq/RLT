...
```{r}
  RLTfit <- RLT(X, Y, Censor, model = "survival", 
                ntrees = 10000, nmin = 40, mtry = 20, split.gen = "random",
                resample.prob = 0.5, resample.replace = FALSE, nsplit = 5,
                param.control = list(split.rule = "logrank", "var.ready" = TRUE), 
                importance = TRUE, verbose = TRUE, ncores = 10)

  RLTPred <- predict(RLTfit, testx, ncores = 10, var.est = TRUE)
  format(object.size(RLTPred), units = "MB")
  
  Band = get.surv.band(RLTPred, alpha = 0.05)
```

```{r}
  par(mfrow = c(1, 2))
  plot(log(1 + timepoints), SurvMat[1, ], type = "l", lwd = 2, col = "darkorange", ylab = "Subject 1")
  lines(log(1 + timepoints), Band[[1]][, 1], type = "l", lty = 2, lwd = 2, col = "deepskyblue")
  lines(log(1 + timepoints), Band[[1]][, 2], type = "l", lty = 2, lwd = 2, col = "deepskyblue")
  
  
  plot(log(1 + timepoints), SurvMat[2, ], type = "l", lwd = 2, col = "darkorange", ylab = "Subject 2")
  lines(log(1 + timepoints), Band[[2]][, 1], type = "l", lty = 2, lwd = 2, col = "deepskyblue")
  lines(log(1 + timepoints), Band[[2]][, 2], type = "l", lty = 2, lwd = 2, col = "deepskyblue")
```
