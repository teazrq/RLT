...
//  Miscellaneous Surv Functions
//  **********************************

// my header file
# include "Surv_Uni_Function.h"
                    ...
                    size_t nmin)
{
  ...
  if ( lowcount < nmin and highcount >= nmin ) // only need to fix lowindex
  {
    ...
    return;
    //RLTcout << "        --- case 1 with lowindex " << lowindex << " highindex " << highindex << std::endl;
  }
  
  if ( lowcount >= nmin and highcount < nmin ) // only need to fix highindex
  {
    while( highcount < nmin and lowindex <= highindex ){
      //RLTcout << "        --- adding " << cat_reduced[highindex].count << " count to highcount " << highcount << std::endl;
    ...
    //RLTcout << "        --- case 2 with lowindex " << lowindex << " highindex " << highindex << std::endl;    
  ...
  if ( lowcount < nmin and highcount < nmin ) // if both need to be fixed, start with one randomly
  {
    ...
    { // fix lowindex first
      ...
      //RLTcout << "        --- case 3 with lowindex " << lowindex << " highindex " << highindex << std::endl;
      return;
      ...
      //RLTcout << "        --- case 4 with lowindex " << lowindex << " highindex " << highindex << std::endl;
      return;
    }
  }
}
