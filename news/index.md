...
- CRAN submission cleanup: fixed vignette pre-building, DESCRIPTION
  fields, man page examples, and C++ compiler warning.

## RLT 6.1.0

- Added OOB (out-of-bag) self-kernel to
  [`forest.kernel()`](https://teazrq.github.io/RLT/reference/forest.kernel.md)
  via `oob = TRUE`. Computes kernel co-occurrence only from trees where
  both observations are OOB, eliminating response-contamination bias for
  unbiased degrees-of-freedom estimation. Returns `Kernel` (normalized),
  `N` (OOB count), and `C` (leaf-sharing count).
